...
// let requestName;  //每次发起请求都会携带这个参数，用于标识这次请求，如果值相等，则取消重复请求
axios.interceptors.request.use((config) => {
  NProgress.start() // start progress bar

  const meta = (config.meta || {});
  const isToken = meta.isToken === false;
  // console.log('env.NODE_ENV2222', process.env.NODE_ENV);
  ...
  //     axios[requestName] = {};
  //     axios[requestName].cancel = c; //取消请求操作
  //   });
  // }
...
axios.interceptors.response.use(res => {
  ...
  if (status === 401) store.dispatch('FedLogOut').then(() => {
    ...
    sessionStorage.removeItem('tabParent'); //清空顶部导航信息
  });
  // 如果请求为非200否者默认统一处理
  if (status !== 200) {
    Message({
